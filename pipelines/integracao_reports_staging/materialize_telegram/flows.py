...
with Flow(
    name="CIVITAS: integracao_reports_staging - Materialize telegram",
    state_handlers=[
        handler_inject_bd_credentials,
        handler_initialize_sentry,
        handler_skip_if_running,
    ],
) as materialize_integracao_reports_telegram:

    dataset_id = Parameter("dataset_id", default="integracao_reports_staging")
    table_id = Parameter("table_id", default="reports_telegram")
...
materialize_integracao_reports_telegram.storage = GCS(constants.GCS_FLOWS_BUCKET.value)
materialize_integracao_reports_telegram.run_config = KubernetesRun(
