...
with Flow(
    name="CIVITAS: DataLake - Carga de dados históricos no datalake do Disque Denúncia",
    state_handlers=[handler_inject_bd_credentials, handler_initialize_sentry],
) as dump_historico_disque_denuncia:
    ...
    dump_prod_tables_to_materialize_parameters = [
        {"dataset_id": "disque_denuncia", "table_id": "denuncias_historico", "dbt_alias": False},
    ]
...
dump_historico_disque_denuncia.storage = GCS(constants.GCS_FLOWS_BUCKET.value)
dump_historico_disque_denuncia.run_config = KubernetesRun(
